...
class UserManager(BaseUserManager):
    def create_user(self, email, wallet_address, password=None, **kwargs):
        
        if not email:
            raise ValueError("Email is required")

        if not wallet_address:
            raise ValueError("Wallet adress is required")
...
class User(AbstractBaseUser):
    ...
    wallet_address = models.CharField(max_length = 100, null = False, blank = False, default = 'Default wallet adress')
    ...
    REQUIRED_FIELDS = ['first_name', 'last_name', 'wallet_address']
